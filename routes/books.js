...
router.get('/',async(req,res,next)=>{
 ...
 router.post("/books",async(req,res,next)=>{
    try{
        ...
        const result=await db.query(
            ...
            [title,author,isbn || null,openlibrary_id ||null,cover_url,rating|| 0, review || null]
 ...
 router.put('/books/:id', async(req,res,next)=>{
    try{
        ...
        await db.query(
            `UPDATE books SET
            title=$1,author=$2,isbn=$3,openlibrary_id=$4,
            cover_url=$5,rating=$6,review=$7,updated_at=now()
            WHERE id=$8`,
        [title,author,isbn,openlibrary_id,cover_url,rating,review,req.params.id]);
...
router.delete('/books/:id', async (req,res,next)=>{
    try{
        await db.query(
            `DELETE FROM books WHERE id=$1`,[req.params.id]
        );
        res.redirect('/');
    }catch(err){
        next(err);
